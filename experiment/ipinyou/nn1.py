...
train_csv_filename = './data/ipinyou/csv/train.csv'
test_csv_filename = './data/ipinyou/csv/test.csv'
vocabulary_folder = './file/ipinyou/vocabulary/'
vocabulary_threshold = 100
...
# Evaluation Function
df_label = pd.read_csv('./data/ipinyou/csv/test.csv', usecols=['click'])


def ipinyou_eval(pred):
    val_logloss = logloss(
        pred,
...
xDeepInt_Model = tf.estimator.Estimator(
    xDeepInt,
    model_dir='./file/ipinyou/nn/xdeepint/',
    params={
        'feature_columns': (
            categorical_feature_categorical_columns
        ),
...
xDeepInt_Model.train(
    input_fn=lambda: train_input_fn(batch_size=1024 * 1, num_epochs=1)
)
